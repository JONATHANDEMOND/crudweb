...
@Component({
  selector: 'app-scanners-sr',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './scanners-sr.component.html',
  styleUrls: ['./scanners-sr.component.css']
})
export class ScannersSrComponent implements OnInit {
  ...
  cargarDatos() {
    this.servicio.getScannersSR().subscribe({
      next: (res: any) => {
  ...
  guardar(form: NgForm) {
    ...
    if (esEdicion && idParaEditar) {
      this.servicio.updateScannerSR(idParaEditar, cuerpo).subscribe({
        next: () => {
      ...
      this.servicio.createScannerSR(cuerpo).subscribe({
        next: () => {
  ...
  eliminar(id: string) {
    if (confirm('¿Está seguro de eliminar este registro?')) {
      this.servicio.deleteScannerSR(id).subscribe({
        next: () => {
  ...
  marcarMantenimiento(item: any) {
    ...
    const cuerpoActualizado = {
      ...
      estado: item.estado,
      mantenimientoRealizado: item.mantenimientoRealizado,
      fechaUltimoMantenimiento: item.fechaUltimoMantenimiento,
      historialMantenimientos: item.historialMantenimientos
    };

    // Corregido al método específico de Scanner Sitios Remotos
    this.servicio.updateScannerSR(item._id, cuerpoActualizado).subscribe({
      next: () => {
  ...
  limpiarMantenimiento(item: any) {
    ...
    if (confirm('¿Desea limpiar el historial de mantenimientos de este equipo?')) {
      ...
      this.servicio.updateScannerSR(item._id, cuerpoLimpio).subscribe({
        next: () => {
        ...
        error: (err) => {
          ...
          alert("No se pudo limpiar el mantenimiento.");
        }
      });
    }
  }
  ...
  private ejecutarGuardadoBaja() {
    ...
    this.servicio.updateScannerSR(idBaja, cuerpoBaja).subscribe({
      next: () => {
